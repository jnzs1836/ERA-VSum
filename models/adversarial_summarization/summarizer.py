...
from ..stgcn.difference_attention import DifferenceAttention

class sLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=2, use_diff=False):
        """Scoring LSTM"""
        super().__init__()
        ...
        self.out = nn.Sequential(
            ...
            nn.Sigmoid())
        if self.use_diff:
            self.diff_attn = DifferenceAttention(init_hidden=False)
        else:
            self.diff_attn = None
    ...
    def forward(self, features, init_hidden=None):
        ...
        h_features, (h_n, c_n) = self.lstm(features)

        # [seq_len, 1]

        scores = self.out(h_features.squeeze(1))

        if self.use_diff:
            diff_scores = self.diff_attn(features)
...
class eLSTM(nn.Module):
    ...
    def forward(self, frame_features):
        ...
        """
        self.lstm.flatten_parameters()
        h, (h_last, c_last) = self.lstm(frame_features)

        return (h_last, c_last, h)
...
class VAE(nn.Module):
    ...
    def forward(self, features):
        ...
        return h_mu, h_log_variance, decoded_features, ht


class Summarizer(nn.Module):
    def __init__(self, input_size, hidden_size, noise_dim=0, num_layers=2, use_diff=False):
        super().__init__()
        self.s_lstm = sLSTM(input_size + noise_dim, hidden_size, num_layers, use_diff=use_diff)
    ...
    def forward(self, image_features, uniform=False, z=None, images=None, video_key=None):
        ...
        if not uniform:
            # [seq_len, 1]
            scores = self.s_lstm(image_features_noise)
